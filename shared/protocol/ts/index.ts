...
/**
 * POSITION_REPORT (C -> S -> other room members). 1 Hz non-authoritative
 * snapshot of the sender's local playback state. The server is a pure relay
 * per the roadmap P4-T03 "server forwards without modification" requirement
 * and per docs/ARCHITECTURE.md section 12.8; the server stamps user_id from
 * the validated bearer but otherwise forwards the original payload verbatim.
 * The payload is not persisted in room state and is not part of
 * PlaybackBookkeeping. The receiving client's UI uses it for display
 * (multi-viewer position indicators) only; it is not applied to the local
 * video element (architecture section 12.3: "Affects local playback: No").
 */
export type PositionReportPayload = { 
/**
 * Originating participant's user_id. Set by the server from the validated
 * bearer so the receiver can attribute the report to a sender without
 * trusting an unverified client field. The wire shape mirrors architecture
 * section 13.1 (the JSON example includes user_id).
 */
user_id: string, 
/**
 * Local video position in milliseconds (rounded from the floating-point
 * currentTime seconds). The wire unit is integer ms so the server can
 * re-emit the value verbatim without precision drift. 0 is a valid value
 * (the very first frame); negative values are not produced by any
 * conforming client.
 */
media_position_ms: bigint, 
/**
 * true when the local video element's paused property is false (i.e. the
 * media element is actively playing); false when the element is paused.
 * playing is observed locally and is not asserted by the server.
 */
playing: boolean, 
/**
 * Sender's wall clock at send (unix ms). Informational only; the server does
 * not stamp server_ts_ms on the rebroadcast (the roadmap explicitly says
 * "server forwards without modification"). Receivers may use this for
 * display only (e.g. "last seen 3 s ago").
 */
client_ts_ms: bigint, };