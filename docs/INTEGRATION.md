...
- **wait_for_complete** (`test.log`). The most common failure here is `state=Transferring transferred=0`. P3-T15 wired the host-side `SenderSession` spawn in `transfer::host_dispatch::HostSenderDispatcher` and `WebRtcManager::on_inbound_data_channel`; the dispatch resolves the source file from the verified manifest and reads chunks from `<library_root>/<media_items.relative_path>`. The viewer-side `WebRtcTransport` splits outgoing frames into <= 16 KiB segments (the webrtc 0.20 `DataChannel` silently drops `OnMessage` events larger than 16 KiB without the detach API) and reassembles them on the receive side. The current remaining failure is a tokio runtime starvation: the multi-source main loop is not draining the inbound frame queue fast enough on a 4-worker-thread runtime. The fix is to increase `worker_threads` from 4 to 8+ in the smoke test config, or move the main loop to a dedicated task with its own priority. See `apps/client/src-tauri/src/transfer/host_dispatch.rs` for the sender wire-up and `apps/client/src-tauri/src/transfer/webrtc_transport.rs` for the segmentation/reassembly implementation.
