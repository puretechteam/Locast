...
- **wait_for_complete** (`test.log`). The most common failure here is `state=Transferring transferred=0`. This means the viewer reached the orchestrator, opened the `DOWNLOAD_OFFER` wire frame on its DataChannel, but no chunks arrived. The host side has no `SenderSession` spawn point wired into `WebRtcManager::on_inbound_data_channel`. This is a P3-T13/P3-T07 production gap, not a smoke-test bug: the wire layer wires the viewer's `MultiSourceReceiver` but does not wire the host's `SenderSession` to listen for `DOWNLOAD_OFFER`. The fix is to add a `SenderSession::run` spawn in the inbound DC handler that reads the verified manifest, builds a `DownloadPlan` from the manifest's media entry, and dispatches chunks back over the same DataChannel. See `tests/transfer_e2e.rs` for the canonical loopback pattern. The smoke test surfaces this gap correctly; the test cannot reach `final_sha256 == source_sha256` until the host side is implemented.
